...
test("isRateLimitError detects rate limit keywords", () => {
  assert.ok(isRateLimitError("Error: rate limit exceeded"));
  assert.ok(isRateLimitError("API is currently overloaded"));
  assert.ok(isRateLimitError("HTTP 529 error"));
  assert.ok(!isRateLimitError("SyntaxError: unexpected token"));
...
test("getFallbackRuntime returns next available runtime in preference order", () => {
  const prefs = { frontend: ["claude-code", "codex"], backend: ["codex", "claude-code"] };
  assert.equal(getFallbackRuntime("frontend", "claude-code", ["codex"], prefs), "codex");
  assert.equal(getFallbackRuntime("frontend", "codex", ["codex"], prefs), null);
  assert.equal(getFallbackRuntime("frontend", "gemini", ["codex"], prefs), null);
});

// ─── selectRuntimeAdaptive ───────────────────────────────────────────────────

test("selectRuntimeAdaptive uses preference order without enough learning data", () => {
  const prefs = { frontend: ["claude-code", "codex"], backend: ["codex", "claude-code"] };
  assert.equal(selectRuntimeAdaptive("frontend", ["claude-code", "codex"], [], prefs), "claude-code");
});

test("selectRuntimeAdaptive reorders by success rate when enough samples exist", () => {
  const prefs = { frontend: ["claude-code", "codex"] };
  // codex: 3 success, claude-code: 0 success 3 fail → codex wins
  const entries = [
